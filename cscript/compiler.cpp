...
namespace
  ...
  void compile_neq(VM::vmcode& code, compile_data& data)
    ...
    if (offset2)
      ...
      code.add(VM::vmcode::XOR, op1, VM::vmcode::NUMBER, 1);
  ...
  void compile_assignment_pointer(VM::vmcode& code, compile_data& data, const Assignment& a)
    ...
    switch (a.op.front())
      ...
      if (rt == RT_REAL)
        ...
        code.add(VM::vmcode::ADDSD, RESERVED_REAL_REG, SECOND_TEMP_REAL_REG);
      ...
      if (rt == RT_REAL)
        {
        code.add(VM::vmcode::MOV, RESERVED_REAL_REG, STACK_MEM_BACKUP_REGISTER);
        code.add(VM::vmcode::SUBSD, RESERVED_REAL_REG, SECOND_TEMP_REAL_REG);
        code.add(VM::vmcode::MOV, STACK_MEM_BACKUP_REGISTER, RESERVED_REAL_REG);
        }
      else
        {
      ...
      if (rt == RT_REAL)
        {
        code.add(VM::vmcode::MOV, RESERVED_REAL_REG, STACK_MEM_BACKUP_REGISTER);
        code.add(VM::vmcode::MULSD, RESERVED_REAL_REG, SECOND_TEMP_REAL_REG);
        code.add(VM::vmcode::MOV, STACK_MEM_BACKUP_REGISTER, RESERVED_REAL_REG);
        }
      else
        {
      ...
      if (rt == RT_REAL)
        {
        code.add(VM::vmcode::MOV, RESERVED_REAL_REG, STACK_MEM_BACKUP_REGISTER);
        code.add(VM::vmcode::DIVSD, RESERVED_REAL_REG, SECOND_TEMP_REAL_REG);
        code.add(VM::vmcode::MOV, STACK_MEM_BACKUP_REGISTER, RESERVED_REAL_REG);
        }
      else
        {
  ...
  void compile_assignment_array(VM::vmcode& code, compile_data& data, const Assignment& a)
    ...
    switch (a.op.front())
      ...
      if (rt == RT_REAL)
        ...
        code.add(VM::vmcode::SUB, STACK_BACKUP_REGISTER, FIRST_TEMP_INTEGER_REG);
    ...
    if (std::holds_alternative<Variable>(lvo.lvalue->lvalue))
      ...
      if (rt == RT_INTEGER)
        ...
        if (it->second.at == memory_address)
          ...
          if (offset)
            ...
            code.add(VM::vmcode::MOV, op, VM::vmcode::MEM_RSP, -(int64_t)var_id);
          double f = 1.0;
          code.add(VM::vmcode::MOV, RESERVED_REAL_REG_2, VM::vmcode::NUMBER, *(reinterpret_cast<uint64_t*>(&f)));
          if (lvo.name == "++")
      ...
      if (it->second.vt == single_array)
        ...
        if (offset)
          ...
          code.add(VM::vmcode::SHL, VM::vmcode::MEM_RSP, offset, VM::vmcode::NUMBER, 3);
          code.add(VM::vmcode::MOV, STACK_BACKUP_REGISTER, VM::vmcode::RSP);
          code.add(VM::vmcode::SUB, STACK_BACKUP_REGISTER, VM::vmcode::MEM_RSP, offset);
          }
        else
          {
          code.add(VM::vmcode::SHL, op, VM::vmcode::NUMBER, 3);
